...
class ObjectDetection:
    def __init__(self,net ='VGGnet_test',model =os.path.join(local_dir,'models/VGGnet_fast_rcnn_iter_150000.ckpt') ):
        # init session
    ...
    def vis_detections(self, im, class_name, dets, thresh=0.5):
        ...
        for i in inds:
            ...
            score = dets[i, -1]
            cv2.rectangle(im,(bbox[0], bbox[1]),(bbox[2], bbox[3]),(255,0,0),thickness=2)
            cv2.putText(im,'{:s} {:.3f}'.format(class_name, score),(bbox[0], bbox[1] - 2),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,255),2)

        return im
    def detect(self,img,ret =dict(),net ='VGGnet_test',model =os.path.join(local_dir,'models/VGGnet_fast_rcnn_iter_150000.ckpt')):
        """Detect object classes in an image using pre-computed object proposals."""
...
if __name__ == '__main__':
    ...
    img = detector.detect(im_names[0])
